...
#  Requires: govc, ssh key access to Ubuntu probe VM (2 NICs)
# ============================================================
...
VM_USER="ubuntu"
VM_MGMT_IP="10.0.0.50"       # management NIC IP to SSH into (static or DHCP-assigned)
MGMT_IFACE="ens160"           # management NIC interface name on the probe VM
...
IFACE="ens192"                # test NIC - gets rotated between VLANs
SSH_KEY="~/.ssh/id_rsa"       # SSH key for probe VM

# --- DNS server to test against (per VLAN or global) ---
DNS_SERVER="8.8.8.8"
DNS_HOSTNAME="google.com"

# --- HTTP/S endpoints to test per VLAN (optional, leave empty to skip) ---
# Format: "url|expected_http_code"
HTTP_TESTS=(
...
# --- Extra hosts to ping per VLAN (beyond gateway) ---
...
SSH_CMD="ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o ConnectTimeout=10 $VM_USER@$VM_MGMT_IP"
PASS="PASS"
FAIL="FAIL"
WARN="WARN"
...
setup_mgmt_nic() {
  ...
  if [ "$MGMT_MODE" = "static" ]; then
    MGMT_OUTPUT=$(run_remote "
      ...
      echo done
    ")
    if echo "$MGMT_OUTPUT" | grep -q "done"; then
      pass "Management NIC configured: $MGMT_STATIC_IP via $MGMT_STATIC_GW"
    else
      echo "ERROR: Failed to configure management NIC in static mode: $MGMT_OUTPUT"
      exit 1
    fi
    ...
    MGMT_OUTPUT=$(run_remote "
      ...
      ip addr show $MGMT_IFACE | grep 'inet ' | awk '{print \$2}'
    ")
    if echo "$MGMT_OUTPUT" | grep -qP '\d+\.\d+\.\d+\.\d+'; then
      pass "Management NIC got DHCP lease: $MGMT_OUTPUT"
    else
...
log "Pre-flight: checking SSH to $VM_MGMT_IP..."
if ! $SSH_CMD "echo ok" &>/dev/null; then
  echo "ERROR: Cannot SSH to $VM_MGMT_IP. Check key/connectivity."
  exit 1
fi
...
log "Pre-flight: installing test tools on probe VM if needed..."
run_remote "which nmap &>/dev/null || sudo apt-get install -y nmap dnsutils curl &>/dev/null"
...
echo ""
echo "  VM      : $VM_NAME"
echo "  Mgmt IP : $VM_MGMT_IP ($MGMT_MODE)"
echo "  Test NIC: $IFACE"
echo ""
...
declare -A HTTP_MAP
for entry in "${HTTP_TESTS[@]}"; do
  IFS='|' read -r url code <<< "$entry"
  HTTP_MAP[$url]="$code"
done
...
for i in "${!VLANS[@]}"; do
  ...
  # 1. Switch port group
  log "Switching NIC to port group: $PG"
  ...
  # 2. Re-IP the interface
  log "Reconfiguring $IFACE to $IP..."
  REIP_OUTPUT=$(run_remote "
    ...
    echo done
  ")
  if echo "$REIP_OUTPUT" | grep -q "done"; then
    pass "Interface configured: $IP via $GW"
    append_result "$PG" "$DESC" "Interface Config" "$PASS" "$IP on $IFACE, gw $GW"
  else
    fail "Interface config issue: $REIP_OUTPUT"
    append_result "$PG" "$DESC" "Interface Config" "$WARN" "$REIP_OUTPUT"
  fi
  sleep 2

  # 3. Ping gateway
  log "Ping gateway $GW..."
  PING_OUT=$(run_remote "ping -c 4 -W 2 $GW 2>&1")
  ...
  if [ "$PING_LOSS" = "0" ]; then
    ...
    pass "Gateway partially reachable ($PING_LOSS% loss)"
  ...
  if [ -n "${EXTRA_MAP[$PG]}" ]; then
    for HOST in ${EXTRA_MAP[$PG]}; do
      ...
      EPING=$(run_remote "ping -c 3 -W 2 $HOST 2>&1")
  ...
  DNS_OUT=$(run_remote "dig @$DNS_SERVER $DNS_HOSTNAME +short +time=3 2>&1")
  ...
  # 6. HTTP tests (matched to this VLAN index)
  HTTP_URL=$(echo "${HTTP_TESTS[$i]}" | cut -d'|' -f1)
  HTTP_EXPECTED=$(echo "${HTTP_TESTS[$i]}" | cut -d'|' -f2)
  if [ -n "$HTTP_URL" ]; then
    log "HTTP test: $HTTP_URL (expect $HTTP_EXPECTED)..."
    HTTP_OUT=$(run_remote "curl -s -o /dev/null -w '%{http_code}' --connect-timeout 5 $HTTP_URL 2>&1")
  ...
  # 7. Port scan gateway (common ports)
  log "Port scan gateway $GW (top 20 ports)..."
  NMAP_OUT=$(run_remote "sudo nmap -T4 --top-ports 20 $GW 2>&1")
  ...
  # 8. MTU / large packet test
  log "MTU test (ping with 1400 byte payload)..."
  MTU_OUT=$(run_remote "ping -c 2 -W 2 -M do -s 1400 $GW 2>&1")
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .stat-card.s-pass .value {{ color: var(--pass); }}
  .stat-card.s-fail .value {{ color: var(--fail); }}
  ...
  .pg-header:hover {{ background: #21262d; }}
  .pg-name {{
    font-family: 'IBM Plex Mono', monospace;
    font-weight: 600;
    color: var(--accent);
  ...
  .test-row {{ background: var(--bg); }}
  .test-row td {{ font-size: 13px; }}
  .status-pill {{
  ...
  .badge-pass {{ background: rgba(63,185,80,0.1); color: var(--pass); }}
  .badge-fail {{ background: rgba(248,81,73,0.1); color: var(--fail); }}
  .badge-warn {{ background: rgba(210,153,34,0.1); color: var(--warn); }}
  footer {{
    margin-top: 40px;
    padding-top: 20px;
...
<header>
  <div>
    <h1>⬡ vSphere VLAN Test Report</h1>
    <div style="color:var(--muted);font-size:13px;margin-top:4px;">VM: $VM_NAME &nbsp;·&nbsp; Host: $VM_MGMT_IP</div>
  </div>
  <div class="meta">
    Generated: {ts}<br>
    VLANs tested: {len(groups)}<br>
    Total checks: {total}
  </div>
</header>
